...
{
    cout<<endl;
    pvec_gloscope = new vector<GloScope*>;   //指向GloScope的vector
    pstack = new stack< vector<Scope*>* >;             //以后都是用pst
    pstack->push((vector<Scope*>*)pvec_gloscope);        //将Global入栈

    for(int i=0;i<pdecllist->size();i++)
    {
        (*pdecllist)[i]->buildsym(pstack,NULL); //第一个参数是栈，第二个是没有初始化的指针
    }
    pstack->pop();
    
}
void Decl::buildsym(stack<vector<Scope*>*> *pst,GloScope* pgloscope)
{

}
void ClassDecl::buildsym(stack<vector<Scope*>*> *pst,GloScope* _pgloscope)
{
//cout<<"Cla"<<endl;
    vector<GloScope*> *pvec_gloscope;
    pvec_gloscope = (vector<GloScope*>*)pst->top();
    //ClaScope域
    vector<ClaScope*> *pvec_clascope;
    pvec_clascope = new vector<ClaScope*>;
    ...
    {
        if(idname==(*pvec_gloscope)[i]->getname())
        {
    ...
    {
        string parentname=base->getidname();
        int i;
